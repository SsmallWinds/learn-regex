...
正先行断言(?=)和负先行断言(?!)表示匹配的内容后面必须为'xxx'或者必须不为'xxx'，即以'xxx'结尾
正先行断言：`(T|t)he(?=\sfat)` 表示匹配'The'或'the' 且 后缀必须为 ‘\sfat’,即以 '\sfat' 结尾
负先行断言：`(T|t)he(?!\sfat)` 表示匹配'The'或'the' 且 后缀必须不为 ‘\sfat’,即不以 '\sfat' 结尾

正后发断言(?<=)和负后发断言(?<!)表示匹配的内容前面必须为'xxx'或者必须不为'xxx'，即以'xxx'开头
正后发断言：'(?<=(T|t)he\s)(fat|mat)' 表示匹配'fat'或'mat' 且 前缀必须为 'The\s'或'the\s',即以 'The\s'或'the\s'开头
负后发断言：'(?<!(T|t)he\s)(fat|mat)' 表示匹配'fat'或'mat' 且 前缀必须不为 'The\s'或'the\s',即不以 'The\s'或'the\s'开头
